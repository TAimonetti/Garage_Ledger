package com.garageledger.data.backup
...
class BackupFileStoreTest {
    ...
    fun pruneOldBackups_keepsNewestZipFilesOnly() {
        val directory = Files.createTempDirectory("garage-ledger-backups").toFile()
        try {
            val oldest = directory.resolve("garage-ledger-backup-1.zip").apply {
                writeText("oldest")
                setLastModified(1_000L)
            }
            val middle = directory.resolve("garage-ledger-backup-2.zip").apply {
                writeText("middle")
                setLastModified(2_000L)
            }
            val newest = directory.resolve("garage-ledger-backup-3.zip").apply {
                writeText("newest")
