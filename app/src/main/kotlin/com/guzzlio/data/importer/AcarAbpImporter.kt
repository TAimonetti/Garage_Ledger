...
class AcarAbpImporter {
    fun import(inputStream: InputStream): ImportedGarageData {
        ...
        val eventSubtypes = parseEventSubtypes(archiveEntries["event-subtypes.xml"]?.inputStream())
        val serviceTypes = mergeServiceTypes(
            primary = parseServices(archiveEntries["services.xml"]?.inputStream()),
            fallback = eventSubtypes.serviceTypes,
        )
        val expenseTypes = mergeExpenseTypes(
            primary = parseExpenses(archiveEntries["expenses.xml"]?.inputStream()),
            fallback = eventSubtypes.expenseTypes,
        )
        ...
        val issues = vehicleBundle.issues.toMutableList()
        if (vehicleBundle.fillUps.isNotEmpty() && vehicleBundle.services.isEmpty()) {
            issues += issue(
                message = "This backup contains fill-up history but no service history. Some later aCar auto-backups omit service records; try an earlier manual or auto-backup if you expected service history.",
                section = "vehicles.xml",
            )
        }
        if (vehicleBundle.fillUps.isNotEmpty() && vehicleBundle.reminders.isEmpty()) {
            issues += issue(
                message = "This backup contains fill-up history but no reminder rows. Guzzlio will seed reminder defaults from the service catalog after import.",
                section = "vehicles.xml",
            )
        }
    ...
    private fun parseEventSubtypes(inputStream: InputStream?): EventSubtypeCatalog {
        if (inputStream == null) return EventSubtypeCatalog()
        val services = mutableListOf<ServiceType>()
        val expenses = mutableListOf<ExpenseType>()
        readXml(inputStream).rootElement().childElements("event-subtype").forEach { element ->
            when (element.getAttribute("type").trim().lowercase()) {
                "service" -> services += ServiceType(
                    legacySourceId = element.getAttribute("id").toLongOrNull(),
                    name = element.childText("name"),
                    notes = element.childText("notes"),
                    defaultTimeReminderMonths = element.childText("default-time-reminder-interval").toIntOrNull() ?: 0,
                    defaultDistanceReminder = parseLooseDouble(element.childText("default-distance-reminder-interval")),
                )

                "expense" -> expenses += ExpenseType(
                    legacySourceId = element.getAttribute("id").toLongOrNull(),
                    name = element.childText("name"),
                    notes = element.childText("notes"),
                )
        ...
        root.childElements("vehicle").forEach { vehicleElement ->
            ...
            vehicleElement.firstChildElement("service-reminders")?.childElements("service-reminder")?.forEach { reminderElement ->
                reminders += ServiceReminder(
                    ...
                    dueDate = parseBackupDateTime(reminderElement.childText("time-due"))?.toLocalDate(),
                    dueDistance = parseLooseDouble(reminderElement.childText("distance-due")),
            ...
            vehicleElement.firstChildElement("reminders")?.childElements("reminder")?.forEach { reminderElement ->
                if (!reminderElement.getAttribute("event-type").equals("service", ignoreCase = true)) return@forEach
                reminders += ServiceReminder(
                    legacySourceId = reminderElement.getAttribute("id").toLongOrNull(),
                    vehicleId = vehicleLegacyId,
                    serviceTypeId = reminderElement.getAttribute("event-subtype-id").toLongOrNull() ?: 0L,
                    intervalTimeMonths = reminderElement.childText("time-interval").toIntOrNull() ?: 0,
                    intervalDistance = parseLooseDouble(reminderElement.childText("distance-interval")),
                    dueDate = parseBackupDateTime(reminderElement.childText("time-due"))?.toLocalDate(),
                    dueDistance = parseLooseDouble(reminderElement.childText("distance-due")),
                    timeAlertSilent = reminderElement.childText("time-alert-silent").equals("true", ignoreCase = true),
                    distanceAlertSilent = reminderElement.childText("distance-alert-silent").equals("true", ignoreCase = true),
            ...
            vehicleElement.firstChildElement("event-records")?.childElements("event-record")?.forEach { eventElement ->
                val type = eventElement.childText("type").trim().lowercase()
                val dateTime = parseBackupDateTime(eventElement.childText("date")) ?: return@forEach
                val subtypeIds = eventElement.firstChildElement("subtypes")
                    ?.childElements("subtype")
                    ?.mapNotNull { it.getAttribute("id").toLongOrNull() }
                    .orEmpty()
                when (type) {
                    "service" -> services += ServiceRecord(
                        legacySourceId = eventElement.getAttribute("id").toLongOrNull(),
                        vehicleId = vehicleLegacyId,
                        dateTime = dateTime,
                        odometerReading = parseLooseDouble(eventElement.childText("odometer-reading")) ?: 0.0,
                        distanceUnit = preferences.distanceUnit,
                        totalCost = parseLooseDouble(eventElement.childText("total-cost")) ?: 0.0,
                        paymentType = eventElement.childText("payment-type"),
                        serviceCenterName = eventElement.childText("place-name"),
                        serviceCenterAddress = eventElement.childText("place-full-address"),
                        latitude = parseLooseDouble(eventElement.childText("place-latitude")),
                        longitude = parseLooseDouble(eventElement.childText("place-longitude")),
                        tags = splitCommaList(eventElement.childText("tags")),
                        notes = eventElement.childText("notes"),
                        serviceTypeIds = subtypeIds,
                    )

                    "expense" -> expenses += ExpenseRecord(
                        legacySourceId = eventElement.getAttribute("id").toLongOrNull(),
                        vehicleId = vehicleLegacyId,
                        dateTime = dateTime,
                        odometerReading = parseLooseDouble(eventElement.childText("odometer-reading")) ?: 0.0,
                        ...
                        totalCost = parseLooseDouble(eventElement.childText("total-cost")) ?: 0.0,
                        paymentType = eventElement.childText("payment-type"),
                        expenseCenterName = eventElement.childText("place-name"),
                        expenseCenterAddress = eventElement.childText("place-full-address"),
                        latitude = parseLooseDouble(eventElement.childText("place-latitude")),
                        longitude = parseLooseDouble(eventElement.childText("place-longitude")),
                        tags = splitCommaList(eventElement.childText("tags")),
                        notes = eventElement.childText("notes"),
                        expenseTypeIds = subtypeIds,
                    )
                }
            }
    ...
    private fun mergeServiceTypes(
        primary: List<ServiceType>,
        fallback: List<ServiceType>,
    ): List<ServiceType> = mergeCatalog(primary, fallback) { item ->
        CatalogKey(
            legacyId = item.legacySourceId,
            name = item.name,
        )
    }

    private fun mergeExpenseTypes(
        primary: List<ExpenseType>,
        fallback: List<ExpenseType>,
    ): List<ExpenseType> = mergeCatalog(primary, fallback) { item ->
        CatalogKey(
            legacyId = item.legacySourceId,
            name = item.name,
        )
    ...
    private fun <T> mergeCatalog(
        primary: List<T>,
        fallback: List<T>,
        keySelector: (T) -> CatalogKey,
    ): List<T> {
        if (primary.isEmpty()) return fallback
        if (fallback.isEmpty()) return primary
        val merged = LinkedHashMap<CatalogKey, T>()
        primary.forEach { merged[keySelector(it)] = it }
        fallback.forEach { item -> merged.putIfAbsent(keySelector(item), item) }
    ...
    private data class EventSubtypeCatalog(
