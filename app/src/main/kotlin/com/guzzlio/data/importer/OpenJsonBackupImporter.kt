...
import com.guzzlio.data.export.AttachmentPayload
import com.guzzlio.data.export.ExpenseRecordPayload
import com.guzzlio.data.export.ExpenseTypePayload
import com.guzzlio.data.export.FillUpRecordPayload
import com.guzzlio.data.export.FuelTypePayload
import com.guzzlio.data.export.OpenJsonBackupMetadata
import com.guzzlio.data.export.OpenJsonBackupPayload
import com.guzzlio.data.export.PreferencesPayload
import com.guzzlio.data.export.SavedBrowseSearchPayload
import com.guzzlio.data.export.ServiceRecordPayload
import com.guzzlio.data.export.ServiceReminderPayload
import com.guzzlio.data.export.ServiceTypePayload
import com.guzzlio.data.export.TripRecordPayload
import com.guzzlio.data.export.TripTypePayload
import com.guzzlio.data.export.VehiclePartPayload
import com.guzzlio.data.export.VehiclePayload
...
class OpenJsonBackupImporter(
    ...
    fun import(inputStream: InputStream): ImportedGarageData {
        ...
        val payloadBytes = archiveEntries["guzzlio-backup.json"]
            ?: error("Selected archive is missing guzzlio-backup.json.")
        ...
        val payload = runCatching {
            ...
            throw IllegalArgumentException("Selected archive is not a valid Guzzlio backup.", error)
        }
        ...
        return ImportedGarageData(
            ...
            vehicleParts = payload.vehicleParts.mapNotNull { part ->
                ...
                if (mappedVehicleId == null) {
                    issues += issue("Skipped vehicle part '${part.name}' because vehicle ${part.vehicleId} is missing.", "guzzlio-backup.json")
                    null
                } else {
    ...
    val visibleFieldSet = buildSet {
        visibleFields.forEach { key ->
            ...
            if (toggle != null) {
                ...
                issues += issue("Ignored unknown visible field key '$key'.", "guzzlio-backup.json")
            }
        }
    }
...
private fun SavedBrowseSearchPayload.toDomain(issues: MutableList<ImportIssue>): SavedBrowseSearch? {
    ...
    if (trimmedName.isBlank()) {
        issues += issue("Ignored saved browse search with a blank name.", "guzzlio-backup.json")
        return null
    }
    return SavedBrowseSearch(
        ...
        family = family?.let { rawFamily ->
            runCatching { RecordFamily.valueOf(rawFamily) }.getOrElse {
                issues += issue("Ignored unknown saved search record type '$rawFamily'.", "guzzlio-backup.json")
                null
            }
        },
        query = query,
        ...
        tripPaidStatus = tripPaidStatus?.let { rawStatus ->
            runCatching { com.guzzlio.domain.model.BrowseTripPaidStatus.valueOf(rawStatus) }.getOrElse {
                issues += issue("Ignored unknown saved search paid status '$rawStatus'.", "guzzlio-backup.json")
                null
            }
        },
    )
...
private fun ServiceReminderPayload.toDomain(
    ...
    if (mappedVehicleId == null) {
        issues += issue("Skipped reminder $id because vehicle $vehicleId is missing.", "guzzlio-backup.json")
        return null
    }
    val mappedServiceTypeId = serviceTypeSourceKeysByBackupId[serviceTypeId]
    if (mappedServiceTypeId == null) {
        issues += issue("Skipped reminder $id because service type $serviceTypeId is missing.", "guzzlio-backup.json")
        return null
    }
...
private fun FillUpRecordPayload.toDomain(
    ...
    val mappedVehicleId = vehicleSourceKeysByBackupId[vehicleId]
    if (mappedVehicleId == null) {
        issues += issue("Skipped fill-up $id because vehicle $vehicleId is missing.", "guzzlio-backup.json")
        return null
    }
    return FillUpRecord(
        id = id,
...
private fun ServiceRecordPayload.toDomain(
    ...
    val mappedVehicleId = vehicleSourceKeysByBackupId[vehicleId]
    if (mappedVehicleId == null) {
        issues += issue("Skipped service record $id because vehicle $vehicleId is missing.", "guzzlio-backup.json")
        return null
    }
...
private fun ExpenseRecordPayload.toDomain(
    ...
    val mappedVehicleId = vehicleSourceKeysByBackupId[vehicleId]
    if (mappedVehicleId == null) {
        issues += issue("Skipped expense record $id because vehicle $vehicleId is missing.", "guzzlio-backup.json")
        return null
    }
...
private fun TripRecordPayload.toDomain(
    ...
    val mappedVehicleId = vehicleSourceKeysByBackupId[vehicleId]
    if (mappedVehicleId == null) {
        issues += issue("Skipped trip record $id because vehicle $vehicleId is missing.", "guzzlio-backup.json")
        return null
    }
    return TripRecord(
        id = id,
...
private fun AttachmentPayload.toDomain(
    ...
    val mappedVehicleId = vehicleSourceKeysByBackupId[vehicleId]
    if (mappedVehicleId == null) {
        issues += issue("Skipped attachment '$displayName' because vehicle $vehicleId is missing.", "guzzlio-backup.json")
        return null
    }
    val family = runCatching { RecordFamily.valueOf(recordFamily) }.getOrElse {
        issues += issue("Skipped attachment '$displayName' because record family '$recordFamily' is unknown.", "guzzlio-backup.json")
        return null
    }
...
private fun issueAndSkipFillUp(
    ...
    issues += issue("Skipped fill-up $id because '$dateTime' is not a valid ISO date-time.", "guzzlio-backup.json")
    return null
}
...
private fun issueAndSkipService(
    ...
    issues += issue("Skipped service record $id because '$dateTime' is not a valid ISO date-time.", "guzzlio-backup.json")
    return null
}
...
private fun issueAndSkipExpense(
    ...
    issues += issue("Skipped expense record $id because '$dateTime' is not a valid ISO date-time.", "guzzlio-backup.json")
    return null
}
...
private fun issueAndSkipTrip(
    ...
    issues += issue("Skipped trip record $id because '$startDateTime' is not a valid ISO date-time.", "guzzlio-backup.json")
    return null
}
